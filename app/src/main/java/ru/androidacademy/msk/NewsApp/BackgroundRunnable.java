...
//    private final WeakReference<LoadingData> loadingDataRef;
    ...
    public BackgroundRunnable(@NonNull Handler handler, @NonNull NewsRecyclerAdapter newsRecyclerAdapter) {
    ...
    public void run() {
        ...
        if (handler != null) {
            handler.post(new UIRunnable(news, newsRecyclerAdapterRef));
        }
    }
}
